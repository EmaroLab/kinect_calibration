...
script to onpen the ssh connection to the remote terminal. It also baxterise it by running the given script [-b] in the given catkin workspace [-c]. Also, it may run the freenect Kinect driver attached to the ros core shared with Baxter.
...
#printf "adjust tilt angle by using: \t $  rostopic pub /tilt_angle std_msgs/Float64 -- -15"
#printf "adjust pan angle by using: \t $  rostopic pub /robot/head/command_head_pan baxter_core_msgs/HeadPanCommand -- 0.0 100"
...
if [[ "$SSH_GRAPHIC" == "1" ]]; then
	sshpass -p $PASSWORD ssh -t -X $SSH_PATH "./openniBaxterisedTerminal.sh  -f $FRENECT_RUN -i $IP -m $MASTER -c $WS -b $BAXTER -a $BAXTERISE "
else
	sshpass -p $PASSWORD ssh -t $SSH_PATH "./openniBaxterisedTerminal.sh -f $FRENECT_RUN -i $IP -m $MASTER -c $WS -b $BAXTER -a $BAXTERISE "
fi
